...
def get_commander_args():
    ...
    parser.add_argument(
        "--no_update_repo",
        action="store_true",
        help="Just download releases, don't use repo-add to update Arch local repository",
    )
...
def get_github_repos_latest_releases_api_url(user_name: str, repo_name: str) -> str:
    api_url = f"https://api.github.com/repos/{user_name}/{repo_name}/releases/latest"
    return api_url


def get_github_latest_releases_download_url(user_name: str, repo_name: str) -> list:
    ...
    if api_response.status_code != 200:
        print(f"Can not get {api_url}, status code: {api_response.status_code}")
        print(api_response.text)
        return

    api_response_json = json.loads(api_response.text)
    download_urls = [i["browser_download_url"] for i in api_response_json["assets"]]
...
def update_arch_repo(package_path: str, repo_path: str, repo_name: str):
    ...
    subprocess.run(
        f"repo-add {db_path} {os.path.join(package_path, '*.tar.zst')}", shell=True
    )
...
def main():
    ...
    download_urls = get_github_latest_releases_download_url(UserName, GitHubRepoName)

    for url in download_urls:
        file_name = urlparse(url).path.split("/")[-1]
        print(f"Downloading {file_name} to {abs_download_path}.")
        download_file(
            url, os.path.join(abs_download_path, file_name), args.force_download
        )
